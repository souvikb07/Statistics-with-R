...
sfo_feb_flights %>% ggplot(aes(x = arr_delay )) + geom_histogram(binwidth = 50)
```
...
# type your code for Question 3 here, and Knit
 sfo_feb_flights %>% group_by(carrier) %>% summarise(median_sfo = median(arr_delay), iqr_sfo = (IQR(arr_delay)))
```
...
nycflights %>% group_by(month) %>% summarise(mean_nyc = mean(dep_delay)) %>% arrange(desc(mean_nyc))
...
nycflights %>% group_by(month) %>% summarise(median_nyc = median(dep_delay)) %>% arrange(desc(median_nyc))
```
nycflights %>% group_by(month) %>% summarise(median_nyc = median(dep_delay)) %>% arrange(desc(median_nyc))
...
The answer is c others are incorrect based on shape and characteristics of mean and median.
...
nycflights %>% group_by(tailnum) %>% arrange(desc(avg_speed)) %>% select(avg_speed, tailnum)
...
nycflights %>% ggplot(aes(x = avg_speed, y = distance)) + geom_point()
...
nycflights <- nycflights %>% mutate(dep_type = ifelse(dep_delay <=0, "on time", "delayed"))
nycflights %>% summarise(newvar = sum(dep_type == "delayed" & arr_type == "on time") / sum(dep_type == "delayed"))
